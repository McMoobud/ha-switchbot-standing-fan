...
async def async_setup_entry(
    ...
    if isinstance(coordinator.device, switchbot.SwitchbotRelaySwitch2PM):
        ...
        async_add_entities(
            [
                SwitchBotStandingFanHorizontalOscillationSwitch(coordinator),
                SwitchBotStandingFanVerticalOscillationSwitch(coordinator),
            ]
        )
...
class SwitchBotStandingFanHorizontalOscillationSwitch(SwitchbotEntity, SwitchEntity):
    """Horizontal oscillation switch for SwitchBot Standing Fan."""
    ...
    def __init__(self, coordinator: SwitchbotDataUpdateCoordinator) -> None:
        ...
        self._attr_unique_id = f"{coordinator.base_unique_id}_h_oscillation"

    @property
    def is_on(self) -> bool | None:
        """Return whether horizontal oscillation is on."""
        return self._device.get_horizontal_oscillating_state()

    @exception_handler
    async def async_turn_on(self, **kwargs: Any) -> None:
        """Start horizontal oscillation."""
        await self._device.set_horizontal_oscillation(True)
        self.async_write_ha_state()

    @exception_handler
    async def async_turn_off(self, **kwargs: Any) -> None:
        """Stop horizontal oscillation."""
        await self._device.set_horizontal_oscillation(False)
        self.async_write_ha_state()


class SwitchBotStandingFanVerticalOscillationSwitch(SwitchbotEntity, SwitchEntity):
    """Vertical oscillation switch for SwitchBot Standing Fan."""
    ...
    def __init__(self, coordinator: SwitchbotDataUpdateCoordinator) -> None:
        """Initialize."""
        super().__init__(coordinator)
        self._attr_unique_id = f"{coordinator.base_unique_id}_v_oscillation"

    @property
    def is_on(self) -> bool | None:
        """Return whether vertical oscillation is on."""
        return self._device.get_vertical_oscillating_state()

    @exception_handler
    async def async_turn_on(self, **kwargs: Any) -> None:
        """Start vertical oscillation."""
        await self._device.set_vertical_oscillation(True)
        self.async_write_ha_state()

    @exception_handler
    async def async_turn_off(self, **kwargs: Any) -> None:
        """Stop vertical oscillation."""
